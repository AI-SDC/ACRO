...
.. note::
   **New in v0.4.12:** Improved missing values handling — when ``check_missing_values`` is enabled and mean/std calculations silently ignore missing data, outputs are now flagged as ``review`` with an explanatory note rather than ``fail``.

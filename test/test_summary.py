...
def test_full_acro_session_summary_generation(tmp_path):
    ...
    assert matrix["output_0"] == {"grant_type": 1, "inc_activity": 0, "year": 1}
    assert matrix["output_1"] == {"grant_type": 1, "inc_activity": 1, "year": 1}
    assert matrix["output_2"] == {"grant_type": 0, "inc_activity": 1, "year": 1}
    ...
    for key in (SUMMARY_JSON_KEY, SUMMARY_CSV_KEY):
        assert key in results_data["results"]
        res = results_data["results"][key]
        assert res["uid"] == key
        assert res["type"] == "custom"
        assert res["status"] == "review"
        assert SUMMARY_WARNING_COMMENT in res["comments"]
